...
LazyNativeFunctions::convolution_backward_overrideable(
  ...
  if (at::globalContext().userEnabledCuDNN() &&
      torch::lazy::getBackend()->GetDefaultDeviceType()->type() == at::kCUDA) {
    LTC_FN_COUNTER("lazy::");
  ...
  const at::DeviceType device_type =
      torch::lazy::getBackend()->GetDefaultDeviceType();
  if (transposed) {
...
at::Tensor LazyNativeFunctions::empty(
  ...
  const auto device_type = torch::lazy::getBackend()->GetDefaultDeviceType()->type();
